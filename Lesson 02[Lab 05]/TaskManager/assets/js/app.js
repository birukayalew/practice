...
asc.addEventListener('click', increasing);
//   Filter Task 
...
function sorting(){
    var values = document.getElementById('dropdown');
...
function sortByKey() {
    index=0
    let arr2 = new Array();
    const search1= document.querySelector('.collection');
    const items1=search1.getElementsByTagName('li')
    const ans = Array.from(items1)
    ans.forEach(function(dates){
        let a = ans[index].firstChild.nextSibling
